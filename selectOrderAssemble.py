import flet as ft
from flet import Page, DataTable, Text, TextButton, View, DataRow, DataCell, IconButton, icons, AppBar
...
def selectOrderAssembleView(page: ft.Page):
    ...
    cursor = cnx.cursor()
    query = ("SELECT * FROM orderDetails WHERE storeID = (%s) order by timePlaced asc")
    tuple = [(login.storeID)]
    ...
    page.views.append(
        View(
            route='/selectOrder',
            controls=[
                dataTable, AppBar(title=Text('Select Order'), bgcolor= 'BLUE')
            ]
            )  
     )
